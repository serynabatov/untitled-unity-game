...
public class PrototypeHero : MonoBehaviour, IDataPersistence
{
    ...
    void Start()
    {
        ...
        m_wallSensorL1 = transform.Find("WallSensor_L1").GetComponent<Sensor_Prototype>();        m_wallSensorL2 = transform.Find("WallSensor_L2").GetComponent<Sensor_Prototype>();
    ...
    void Update()
    {
        ...
        {
            inputX = InputManager.GetInstance().GetMoveAxis(); //Input.GetAxis("Horizontal");
        }


        // GetAxisRaw returns either -1, 0 or 1
        //float inputRaw = Input.GetAxisRaw("Horizontal"); // Старое
        float inputRaw = InputManager.GetInstance().GetMoveAxis(); // Новое
        ...
        // Parry & parry stance
        if (InputManager.GetInstance().GetParryPressed() && !m_dodging && !m_ledgeGrab && !m_ledgeClimb && !m_crouching && m_grounded)
        {
            // Parry
        ...
        //Up Attack
        else if (InputManager.GetInstance().GetUpAttackPressed() && !m_dodging && !m_ledgeGrab && !m_ledgeClimb && !m_crouching && m_grounded && m_timeSinceAttack > 0.2f)
        {
        ...
        //Attack
        else if (InputManager.GetInstance().GetAttackPressed() && !m_dodging && !m_ledgeGrab && !m_ledgeClimb && !m_crouching && m_grounded && m_timeSinceAttack > 0.2f)
        {
            // Reset timer
            m_timeSinceAttack = 0.0f;
        ...
        //Air Slam Attack
        /*
        else if (InputManager.GetInstance().GetAirSlamPressed() && !m_ledgeGrab && !m_ledgeClimb && !m_grounded)
        {
        ...
        // Air Attack Up
        else if (InputManager.GetInstance().GetAirUpAttackPressed() && !m_dodging && !m_ledgeGrab && !m_ledgeClimb && !m_crouching && !m_grounded && m_timeSinceAttack > 0.2f)
        {
        ...
        // Air Attack
        else if (InputManager.GetInstance().GetAirAttackPressed() && !m_dodging && !m_ledgeGrab && !m_ledgeClimb && !m_crouching && !m_grounded && m_timeSinceAttack > 0.2f)
        {
        ...
        // Dodge
        else if (InputManager.GetInstance().GetDodgePressed() && m_grounded && !m_dodging && !m_ledgeGrab && !m_ledgeClimb)
        {
            m_dodging = true;
            m_crouching = false;
        ...
        // Ledge Climb
        else if (InputManager.GetInstance().GetMoveDirection().y == 1 && m_ledgeGrab)
        {
            DisableWallSensors();
            m_ledgeClimb = true;
            m_body2d.gravityScale = 0;
        ...
        //Crouch / Stand up
        else if (InputManager.GetInstance().GetMoveDirection().y == -1 && m_grounded && !m_dodging && !m_ledgeGrab && !m_ledgeClimb && m_parryTimer < 0.0f)
        {
            m_crouching = true;
            m_animator.SetBool("Crouching", true);
            m_body2d.velocity = new Vector2(m_body2d.velocity.x / 2.0f, m_body2d.velocity.y);
        }
        else if (InputManager.GetInstance().GetMoveDirection().y == 0 && m_crouching)
        {
            m_crouching = false;
            m_animator.SetBool("Crouching", false);
        }
