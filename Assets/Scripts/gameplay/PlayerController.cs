...
public class PlayerController : MonoBehaviour
{
    ...
    private void CheckLedgeClimb()
    {
        if (ledgeDetected && !canClimbLedge)
        {
            ...
            {
                //Серый рефакторинг кода
                ledgePos1 = new Vector2(Mathf.Ceil(ledgePosBot.x - wallCheckDistance) + ledgeClimbXOffset1, Mathf.Floor(ledgePosBot.y) + ledgeClimbYOffset1);
                ledgePos2 = new Vector2(Mathf.Ceil(ledgePosBot.x - wallCheckDistance) - playerCollider.size.x / ledgeCollider - noise, Mathf.Floor(ledgePosBot.y) + ledgeClimbYOffset2);
            }
    ...
    private void CheckSurroundings()
    {
        ...
        if (isTouchingWall && !isTouchingLedge && !ledgeDetected)
        {
            ...
            BoxCollider2D detectedCollider = (BoxCollider2D)(Physics2D.OverlapCircle(wallCheck.position, 1f, whatIsGround));
            ledgeCollider = detectedCollider.size.x;
        }
    }
